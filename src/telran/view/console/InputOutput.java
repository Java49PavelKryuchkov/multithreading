...
default LocalDate readDate(String prompt, String errorPrompt, LocalDate from, LocalDate to) {
	return readObject(String.format("%s[%d - %d]", prompt, from, to), errorPrompt, 
			string -> {
				LocalDate res = LocalDate.parse(string);
				if(res.isAfter(to)) {
					throw new IllegalArgumentException("must be before %s date" + to);
				}
				if(from.isBefore(from)) {
					throw new IllegalArgumentException("must be after %s date" + from);
				}
				return res;
			});
}
